...
// TODO: Replace the heightmap data structure with a vector.
// Flattening the 2D coordinates into a 1 dimensional vector should speed
// stuff up by a TON.
...
void Translator::translate(GCodeFile &ir, const TranslatorConfig &config) {
    ...
    for (GCodeCommand &cmd : layer.commands) {
      ...
      if (cmd.type == CommandType::MOVE && cmd.params.count('E')) {
        ...
        for (double s = 0; s <= dist; s += step_size) {
          ...
          // If distance is 0, there is no line to interpolate. This avoids
          // division by 0 t is the interpolation factor. Basically, it is how
          // far along the distance of the line we are. A t of 0.5 means we are
          // halfway along the time (At the midpoint).
          ...
          int gX =
              static_cast<int>(std::floor(interpX / config.grid_resolution));
          int gY =
              static_cast<int>(std::floor(interpY / config.grid_resolution));
          ...
          // If the current nozzle height (curZ) is heigher than what we
          // currently have in that pixel, replace it.
  ...
  std::unordered_map<std::pair<int, int>, double, PairHash> original_heightmap =
      heightmap;
...
// Throwback to trig
void Translator::applyHeightSmoothing(
    std::unordered_map<std::pair<int, int>, double, PairHash> &heightmap,
    const TranslatorConfig &config, int minX, int maxX, int minY, int maxY) {
...
void Translator::applyIRMorphing(
    GCodeFile &ir,
    const std::unordered_map<std::pair<int, int>, double, PairHash> &heightmap,
  ...
  // Basically the same logic as phase 1 BUT instead we are modifying the
  // heightmap.
  for (Layer &layer : ir.layers) {
    for (GCodeCommand &cmd : layer.commands) {
